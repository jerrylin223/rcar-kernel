...
static int rcsi2_d_phy_setting_v4h(struct rcar_csi2 *priv, int mbps)
{
	...
	rcsi2_write16(priv, PPI_STARTUP_RW_COMMON_DPHY(7), mbps > 1500 ? 0x0028 : 0x0068);
	...
	if (mbps > 1500) {
		...
		rcsi2_write16(priv, PPI_RW_DDLCAL_CFG(3), val);
	}

	if (lut) {
		rcsi2_modify16(priv, PPI_RW_DDLCAL_CFG(1),
			       lut->cfg_1, 0x00ff);
		rcsi2_modify16(priv, PPI_RW_DDLCAL_CFG(5),
	...
	for (i = 0; i < ARRAY_SIZE(deskew_fine); i++) {
		rcsi2_write16(priv, CORE_DIG_COMMON_RW_DESKEW_FINE_MEM,
			      deskew_fine[i]);
	}
...
static int rcsi2_start_receiver_v4h(struct rcar_csi2 *priv)
{
	...
	rcsi2_write(priv, PHY_MODE,
		    priv->cphy_connection ? V4H_PHY_MODE_CPHY : V4H_PHY_MODE_DPHY);
	...
	if (priv->cphy_connection) {
		...
		ret = rcsi2_d_phy_setting_v4h(priv, data_rate);
		if (ret) {
	...
	if (rcsi2_wait_phy_start_v4h(priv, ST_PHY_READY)) {
		dev_err(priv->dev, "PHY calibration failed\n");
		return -ETIMEDOUT;
	}
	...
	if (!priv->cphy_connection) {
		u16 val_2_9 = data_rate > 2500 ? 0x14 : data_rate > 1500 ? 0x04 : 0x00;
		u16 val_2_15 = data_rate > 1500 ? 0x03 : 0x00;
...
static int rcsi2_start(struct rcar_csi2 *priv)
{
	...
	/* T7: Wait for stop state (Confirmation of CSI PHY) */
	if (priv->info->features & RCAR_VIN_R8A779G0_FEATURE ||
	    priv->info->features & RCAR_VIN_R8A779H0_FEATURE)
		rcsi2_wait_phy_start_v4h(priv, ST_STOPSTATE_DCK |
					 ST_STOPSTATE_0 | ST_STOPSTATE_1 |
					 ST_STOPSTATE_2 | ST_STOPSTATE_3);
